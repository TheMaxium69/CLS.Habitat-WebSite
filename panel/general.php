...
<main>
    ...
    foreach ($AllEnv as $EnvSection){
        ...
        if ($EnvSection['name'] == "siret"){ $SectionSiret = $EnvSection; }
    }
    ...
    <div class="container py-5">
        ...
        <table class="container py-5">
            <tr>
                <td><?php echo $SectionSiret['content'] ?></td>
                <td><a href="?edit=<?php echo $SectionSiret['name'] ?>"><i class="fas fa-edit"></i></a></td>
            </tr>
        </table>
        <br>
        <form <?php if (empty($_GET['edit']) || $_GET['edit'] != "siret"){ echo "style='display: none'"; } ?> method="GET">
            <input type="hidden" name="name" value="<?php echo $_GET['edit'] ?>">
            <div class="form-group">
                <input type="text" class="form-control" placeholder="Nouvelle Adresse" required name="content">
                <small class="form-text text-muted">Ce siret sera changer sur tout le site</small>
            </div>
            <div class="form-group form-check">
                <input type="checkbox" class="form-check-input" required>
                <label class="form-check-label">Vous comfirmer changer ça sur tout le site</label>
